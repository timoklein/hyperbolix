...
- **`LearnableCurvature(nnx.Module)`** — canonical module for trainable curvature in `hyperbolix.utils.curvature`, also exported at top level (`from hyperbolix import LearnableCurvature`). Supports three reparameterizations:
    - `parameterization="softplus"` (default): `c = softplus(raw)`, bounded gradient via sigmoid (van Spengler 2023 convention)
    - `parameterization="log"`: `c = exp(raw)`, scale-invariant gradient `dc/draw = c` (MERU convention, preferred for compiled RL loops). The exponent is capped at `0.99·log(finfo.max)`, so a drifting raw parameter yields a large finite curvature instead of `inf`; the cap is gradient-transparent under `straight_through_clamp=True`
    - `parameterization="identity"`: `c = raw` — **signed**, crossing zero smoothly with gradient 1, for learning the *sign* of curvature with the `Stereographic` manifold; `init_c` may be zero or negative

    The default clamp is applied to the recovered `c` (not the raw parameter) as a hard stability guard for long compiled training loops, and resolves per parameterization: `[0.1, 10.0]` for the positive `softplus`/`log` schemes, symmetric `[-10.0, 10.0]` (deliberately including 0) for `identity`. Pass `c_min=None, c_max=None` to disable. Updated by any standard Euclidean `nnx.Optimizer` — no Riemannian optimizer needed
