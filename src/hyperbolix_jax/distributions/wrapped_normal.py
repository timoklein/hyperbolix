...
tangent space at the origin onto the manifold.

- Hyperboloid: Uses parallel transport and exponential map
- Poincaré ball: Uses exponential map and Möbius addition (simpler, no parallel transport)
...
def sample_poincare(
    ...
    Examples:
        ...
        >>> sigma = 0.1  # Isotropic
        >>> z = wrapped_normal.sample_poincare(key, mu, sigma, c=1.0)
        >>> z.shape
        (2,)
        >>>
        >>> # Multiple samples with diagonal covariance
        >>> sigma_diag = jnp.array([0.1, 0.2])  # Diagonal
        >>> z = wrapped_normal.sample_poincare(key, mu, sigma_diag, c=1.0, sample_shape=(5,))
        >>> z.shape
        (5, 2)
        >>>
        >>> # Batch of means
        >>> mu_batch = jnp.array([[0.0, 0.0], [0.1, 0.1]])  # (2, 2)
        >>> z = wrapped_normal.sample_poincare(key, mu_batch, 0.1, c=1.0)
        >>> z.shape
        (2, 2)
    """
    # Determine output dtype
    ...
    # Sample from multivariate normal
    mean = jnp.zeros(n, dtype=dtype)
    v = jax.random.multivariate_normal(key, mean, cov, shape=sample_shape, dtype=dtype)
