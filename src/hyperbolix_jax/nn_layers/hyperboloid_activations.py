...
def hyp_relu(x: Float[Array, "... dim_plus_1"], c: float) -> Float[Array, "... dim_plus_1"]:
    ...
    --------
    >>> import jax.numpy as jnp
    >>> from hyperbolix_jax.nn_layers import hyp_relu
    >>>
    >>> # Single point
...
def hyp_leaky_relu(
    ...
    --------
    >>> import jax.numpy as jnp
    >>> from hyperbolix_jax.nn_layers import hyp_leaky_relu
    >>>
...
def hyp_tanh(x: Float[Array, "... dim_plus_1"], c: float) -> Float[Array, "... dim_plus_1"]:
    ...
    --------
    >>> import jax.numpy as jnp
    >>> from hyperbolix_jax.nn_layers import hyp_tanh
    >>>
    >>> # Single point
    >>> x = jnp.array([1.05, 0.1, -0.2, 0.15])
    >>> y = hyp_tanh(x, c=1.0)
...
def hyp_swish(x: Float[Array, "... dim_plus_1"], c: float) -> Float[Array, "... dim_plus_1"]:
    ...
    --------
    >>> import jax.numpy as jnp
    >>> from hyperbolix_jax.nn_layers import hyp_swish
    >>>
    >>> # Single point
    >>> x = jnp.array([1.05, 0.1, -0.2, 0.15])
    >>> y = hyp_swish(x, c=1.0)
