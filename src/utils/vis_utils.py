...
def plot_2d_points(x: torch.Tensor, ax: plt.Axes, labels: Union[npt.ArrayLike, None] = None) -> List[plt.Line2D]:
    ...
    if labels is None:
        ...
        handles = [
            plt.Line2D(
                [0], [0], marker="o", color="w", markerfacecolor=color_map[label], markersize=10, alpha=0.6, label=f"{label}"
            )
...
def plot_hyperplane(
    ...
    handles.append(
        plt.Line2D([0], [0], color="green", marker="P", linestyle="", markersize=10, label="Hyperplane Base Points")
    )
