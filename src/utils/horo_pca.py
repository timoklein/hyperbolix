...
def compute_frechet_mean(x: torch.Tensor, hyperboloid: Hyperboloid) -> torch.Tensor:
    ...
    for lr in [1e-02, 2e-02, 5e-03, 4e-02, 2.5e-03]:
        ...
        print("compute_frechet_mean: No convergence with any learning rate. Using the best candidate mean.", flush=True)
    return mean
...
class HoroPCA(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(self.manifold, PoincareBall):
            ...
            self.Q = nn.Parameter(
                torch.randn(self.n_components, self.n_in_features, dtype=self.manifold.dtype, device=self.manifold.c.device)
            )
        elif isinstance(self.manifold, Hyperboloid):
            self.hyperboloid = self.manifold
            self.Q = nn.Parameter(
                torch.randn(
                    self.n_components, self.n_in_features - 1, dtype=self.manifold.dtype, device=self.manifold.c.device
                )
            )
        else:
