...
from hyperbolix_jax.optim import (
    get_manifold_info,
    mark_manifold_param,
    riemannian_adam,
    riemannian_sgd,
...
class TestManifoldMetadata:
    """Test manifold metadata system."""

    def test_mark_and_retrieve_metadata(self):
    ...
    def test_callable_curvature(self):
...
class TestRiemannianSGD:
    """Test Riemannian SGD optimizer."""

    @pytest.mark.parametrize("use_expmap", [True, False])
    def test_convergence_to_target(self, use_expmap):
...
class TestNNXIntegration:
    """Test integration with nnx.Optimizer and actual layers."""

    def test_hyplinear_poincare_with_rsgd(self):
    ...
    def test_jit_compilation(self):
        ...
        @nnx.jit
        def train_step(model, optimizer_state, x):
            def loss_fn(m):
                y = m(x, c=1.0)
                return jnp.sum(y**2)
...
class TestEdgeCases:
    ...
    def test_params_required_error(self):
