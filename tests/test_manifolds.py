...
from src.manifolds import Euclidean, Hyperboloid, Manifold, PoincareBall


def test_addition(manifold: Manifold, tolerance: tuple[float, float], uniform_points: torch.Tensor) -> None:
    ...
    # Additive identity
    torch.testing.assert_close(manifold.addition(identity, uniform_points), uniform_points, atol=atol, rtol=rtol)
    torch.testing.assert_close(manifold.addition(uniform_points, identity), uniform_points, atol=atol, rtol=rtol)
    # Additive inverse
    torch.testing.assert_close(manifold.addition(-uniform_points, uniform_points) + 1, identity + 1, atol=atol, rtol=rtol)
    torch.testing.assert_close(manifold.addition(uniform_points, -uniform_points) + 1, identity + 1, atol=atol, rtol=rtol)
    # Distributive law
...
def test_scalar_mul(seed: None, manifold: Manifold, tolerance: tuple[float, float], uniform_points: torch.Tensor) -> None:
    ...
    torch.testing.assert_close(manifold.scalar_mul(identity, uniform_points), uniform_points, atol=atol, rtol=rtol)
    # Associative laws
    ...
    if isinstance(manifold, (Euclidean, PoincareBall)):
        ...
        # N-Gyroaddition
        torch.testing.assert_close(n_sum, manifold.scalar_mul(n * identity, uniform_points), atol=atol, rtol=rtol)
        # Distributive laws
        torch.testing.assert_close(
            manifold.scalar_mul(r1 + r2, uniform_points),
            manifold.addition(
        ...
        torch.testing.assert_close(
            manifold.scalar_mul(-r1, uniform_points), manifold.scalar_mul(r1, -uniform_points), atol=atol, rtol=rtol
        )
        # Scaling property
        ...
        torch.testing.assert_close(
            left_side, uniform_points / uniform_points.norm(p=2, dim=-1, keepdim=True), atol=atol, rtol=rtol
        )
        # Homogenity property
        torch.testing.assert_close(
...
def test_dist(manifold: Manifold, tolerance: tuple[float, float], uniform_points: torch.Tensor) -> None:
    ...
    # Symmetry
    torch.testing.assert_close(
        manifold.dist(x, y, version=version), manifold.dist(y, x, version=version), atol=atol, rtol=rtol
    )
    # Triangle inequality
    assert torch.all(
        manifold.dist(x, z, version=version)
        <= manifold.dist(x, y, version=version) + manifold.dist(y, z, version=version) + atol
    )
...
def test_expmap_retraction_logmap(manifold: Manifold, tolerance: tuple[float, float], uniform_points: torch.Tensor) -> None:
    ...
    torch.testing.assert_close(
        manifold.logmap(uniform_points, origin), manifold.logmap_0(uniform_points), atol=atol, rtol=rtol
    )


def test_ptransp(manifold: Manifold, tolerance: tuple[float, float], uniform_points: torch.Tensor) -> None:
    ...
    torch.testing.assert_close(
        manifold.tangent_inner(u, v, origin), manifold.tangent_inner(u_pt, v_pt, uniform_points), atol=atol, rtol=rtol
    )
    # Consistency of ptransp with ptransp_0
    torch.testing.assert_close(manifold.ptransp(u, origin, uniform_points), u_pt, atol=atol, rtol=rtol)
    # Numerical stability
    torch.testing.assert_close(manifold.ptransp(u_pt, uniform_points, origin), u, atol=atol, rtol=rtol)
    assert manifold.is_in_tangent_space(manifold.ptransp(u_pt, uniform_points, origin), origin)


def test_tangent_norm(manifold: Manifold, tolerance: tuple[float, float], uniform_points: torch.Tensor) -> None:
    ...
    torch.testing.assert_close(
        manifold.dist(x, y, version=version), manifold.tangent_norm(manifold.logmap(y, x), x), atol=atol, rtol=rtol
    )
    torch.testing.assert_close(
        manifold.dist_0(uniform_points, version=version),
...
def test_gyration(seed: None, manifold: Manifold, tolerance: tuple[float, float], uniform_points: torch.Tensor) -> None:
    ...
    torch.testing.assert_close(
        manifold.addition(x, y), manifold._gyration(x, y, manifold.addition(y, x)), atol=atol, rtol=rtol
    )
    ...
    # Left loop property
    torch.testing.assert_close(
        manifold._gyration(x, y, z), manifold._gyration(manifold.addition(x, y), y, z), atol=atol, rtol=rtol
    )
    # Right loop property
    torch.testing.assert_close(
        manifold._gyration(x, y, z), manifold._gyration(x, manifold.addition(y, x), z), atol=atol, rtol=rtol
    )
    # Identity gyroautomorphism property
    r1 = torch.rand((x.shape[0], 1), dtype=x.dtype)
    r2 = torch.rand((x.shape[0], 1), dtype=x.dtype)
    torch.testing.assert_close(
        manifold._gyration(manifold.scalar_mul(r1, x), manifold.scalar_mul(r2, x), y), y, atol=atol, rtol=rtol
    )
